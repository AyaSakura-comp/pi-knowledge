...
- `knowledge_search` diagnostics/provenance are a behavior contract. Results should expose chunk id, chunk hash, indexed timestamp, stale flag, match reason, and source freshness when available so agents can distinguish current verified source from stale or weak evidence.
